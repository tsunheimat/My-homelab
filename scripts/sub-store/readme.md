...
| 參數 | 說明 | 範例 |
| --- | --- | --- |
| **`in`** | **輸入格式指定**<br><br> <br><br>預設為自動判斷。若識別不準，可強制指定原始名稱的類型。 <br><br> <br><br>可選值：`zh`(中文), `us`/`en`(英文簡稱), `quan`(英文全名), `flag`(國旗)。 | `#in=us` |
| **`out`** | **輸出格式指定**<br><br> <br><br>重新命名後的國家名格式。 **預設為中文**。 <br><br> <br><br>可選值：`zh`(中文), `us`/`en`(英文簡稱), `quan`(英文全名), `flag`/`gq`(僅限國旗)。 | `#out=us` (輸出 HK, US) |
| **`nm`** | **保留未符合節點**<br><br> <br><br>預設情況下，無法辨識國家的節點會被刪除。新增此參數可保留原名。 | `#nm` |

### 2\. 格式與美化 (前綴/後綴/國旗)

| 參數 | 說明 | 範例 |
| --- | --- | --- |
| **`flag`** | **新增國旗**<br><br> <br><br>在節點名稱最前方加入對應的 emoji 國旗。 | `#flag` |
| **`fgf`** | **分隔符號**<br><br> <br><br>設定前綴、國旗、國家名稱之間的分隔符號。預設為空格。 | \`#fgf= |
| **`sn`** | **序號分隔符號**<br><br> <br><br>設定國家名稱與序號（01, 02）之間的分隔符號。預設為空格。 | `#sn=_` |
| **`name`** | **新增自訂前綴**<br><br> <br><br>在節點名稱最前方（或國旗後）新增自訂名稱（如機場名稱）。 | `#name=我的機場` |
...
| 參數 | 說明 | 範例 |
| --- | --- | --- |
| **`blkey`** | **保留關鍵字**<br><br> <br><br>擷取並保留原名中的特定字元。多個字用 `+` 連接。 <br><br> <br><br>支援重新命名：`舊名>新名`。 | `#blkey=IPLC+GPT>AI` |
| **`blgd`** | **保留固定標識**<br><br> <br><br>開啟內建正規庫，保留 `IPLC`, `IEPL`, `家寬`, `Game` 等常見標識。 | `#blgd` |
| **`bl`** | **保留倍率**<br><br> <br><br>自動正規匹配並保留節點中的倍率（如 `1.5x`, `3倍`）。 | `#bl` |
...
| 參數 | 說明 | 範例 |
| --- | --- | --- |
| **`blpx`** | **特殊節點置頂**<br><br> <br><br>將包含 `blgd` 或 `blkey` 中匹配到的特殊節點（如 IPLC）排在前面。 | `#blpx` |
...
_輸入：_ `US_GPT_Line_05` _輸出：_ `MyProxy-🇺🇸-美國-OpenAI-05` _邏輯：_ 新增前綴 MyProxy，分隔符號 -，將 GPT 改名為 OpenAI，輸出中文國家名稱。
